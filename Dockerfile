...
# --- Config file (XXE target - static, never changes) ---
...
# --- Apache + PHP configuration (serve uploads directory) ---
...
# Apache listens on port 8080 (Flask uses 5000)
RUN sed -i 's/Listen 80/Listen 8080/' /etc/apache2/ports.conf && \
    sed -i 's/<VirtualHost \\*:80>/<VirtualHost *:8080>/' /etc/apache2/sites-available/000-default.conf && \
    printf '<Directory /var/www/html/uploads>\n    Options Indexes FollowSymLinks\n    AllowOverride All\n    Require all granted\n</Directory>\n' >> /etc/apache2/apache2.conf

# Startup script — regenerates SSH keys on every container start
